...
BigInt MulBigInt(BigInt a, BigInt b){
	BigInt novoBig = a;
	for(BigInt i = "1"; i != b; i = addBigInt(i, "1"))
		novoBig = addBigInt(novoBig, a);
	return novoBig;
}
...
pair<BigInt,BigInt> DivideBigInt(BigInt a, BigInt b){ // returns the division and the remainder
	if(a == b) return make_pair("1", "0");
	else if(IsBigger(b,a)) return make_pair("0", a);
	BigInt div = "0", rem = "0";
	while(IsBigger(a,b)){
		a = SubtractBigInteger(a,b);
		div = addBigInteger(div, "1");
	}
	rem = a;
